...
<a href="https://reevl.co.kr/mcp">
  <img src="assets/reevl-icon.svg" alt="Reevl" width="88" height="88">
</a>
...
| `search_apartments` | 지역·브랜드·시공사·가격·세대수·전세가율·AI예측 등으로 아파트를 거른다 |
| `search_properties` | 아파트 **외** 12유형 — 빌라·오피스텔·단독다가구·토지·상가·분양권과 그 전월세, 청약·공공임대·공매 |
| `get_apartment` | 단지 하나의 전 축을 묶음으로 |
| `get_transactions` | 단지 또는 법정동의 **개별 실거래**를 최신순으로(계약일·금액·전용면적·층) |
| `get_loan_rules` | 주담대 현행 규제 수치(LTV·DSR·스트레스 가산금리)를 기준일과 함께. 계산은 하지 않는다 |
| `list_articles` | 리블이 발행한 부동산 브리핑(정책·시장 분석) |

전부 `readOnlyHint: true`이고 쓰기·주문·중개 기능은 없습니다.

> `search_properties`의 청약·공공임대·공매는 **실거래가 아니라 공고 기준**입니다. 그 셋을 실거래로 읽으면 값의 뜻이 달라집니다.
